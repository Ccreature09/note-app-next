...
export const List = ({ listInfo }) => {
  const listID = listInfo && listInfo.listID ? JSON.stringify(listInfo.listID).replace(/['"]+/g, '') : ''
  const listUID = listInfo && listInfo.uid ? JSON.stringify(listInfo.uid).replace(/['"]+/g, '') : ''
  const userInfo = Auth()
  ...
  const addItem = () => {
    ...
    const ListInDB = ref(database, `${userInfo.isAnonymous ? 'guests' : 'users'}/${listUID}/lists/${listID}/items`)
    if (inputValue.trim() === '') {
  ...
  const removeItem = (itemId) => {
    const exactLocationOfItemInDB = ref(
      database,
      `${isAnonymous ? 'guests' : 'users'}/${listUID}/lists/${listID}/items/${itemId}`
    )
    remove(exactLocationOfItemInDB)
  }

  useEffect(() => {
    const listRef = ref(database, `${isAnonymous ? 'guests' : 'users'}/${listUID}/lists/${listID}/items`)
        ...
        <div>
          ...
          <ul className='list-none flex flex-wrap gap-3'>
            {items.map(([itemId, item]) => (
              ...
              >
                <div>{item.name}</div>
                {item.formattedTime && <div className='text-blue-500'>{item.formattedTime}</div>}
              </li>
            ))}
          </ul>
        </div>
