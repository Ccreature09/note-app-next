...
export const Sidebar = ({ setSelectedListID }) => {
  ...
  useEffect(() => {
    if (userInfo) {
      ...
      if (!userInfo.isAnonymous) {
        const uid = ref(database, `${userInfo.isAnonymous ? 'guests' : 'users'}/${userInfo.uid}`)
        update(uid, {
      ...
      const userListsRef = ref(database, `${userInfo.isAnonymous ? 'guests' : 'users'}/${userInfo.uid}/lists`)
  ...
  useEffect(() => {
    ...
    userPartOfLists.forEach((list) => {
      const listRef = ref(database, `users/${sharedListOwner}/lists/${list.id}`)
  ...
  const removeList = (listId) => {
    ...
    let listRef
    if (userInfo) {
      listRef = ref(database, `${userInfo.isAnonymous ? 'guests' : 'users'}/${userInfo.uid}/lists/${listId}`)
    }
  ...
  return (
    <div className={`flex flex-col p-4 bg-[#1D3557] md:h-screen w-full md:w-64 ${poiret_One.variable} font-sans`}>
      <h1 className={` ${monoton.variable} font-mono text-white text-3xl font-medium text-center mb-5 `}>Octonotes</h1>
      ...
      {userInfo && !isAnonymous && (
        <div className='flex items-center justify-center space-x-4 mb-6'>
          <Image className='rounded-full' src={img} width={64} height={64} alt='Picture of the author' />
          <div className='flex flex-col'>
            <p className='text-[#F1FAEE] font-black text-2xl'>{name}</p>
            <p className='text-[#F1FAEE] text-base truncate md:w-auto'>{email}</p>
          </div>
        </div>
      )}
          ...
          <div>
            ...
            {userLists.length > 0 && (
              <div className={`${ListCollapsed ? 'hidden' : ''}`}>
                <p className={'text-[#F1FAEE] font-bold text-2xl mb-2 text-center'}>My Lists:</p>
                <hr />
                <br />
                <ul>
                  {userLists.map((list) => (
                    <div
                      className={`flex items-center justify-between p-4 ${ListCollapsed ? 'hidden' : ''}`}
                      key={list.id}
                    >
                      <li
                        className={`transition-all duration-200 cursor-pointer text-2xl font-black p-3 mb-3 rounded-lg flex-grow text-center  text-[#F1FAEE] max-w-2xl overflow-auto ${
                          activeListItem === list.id ? 'bg-[#457B9D]' : 'bg-[rgba(0,0,0,0.15)] hover:bg-[#457B9D]'
                        }`}
                        onClick={() => {
            ...
            {userPartOfLists.length > 0 && (
              <div className={`${ListCollapsed ? 'hidden' : ''}`}>
                <p className={`text-[#F1FAEE] font-bold text-2xl mb-2 text-center `}>Shared Lists:</p>
                <hr />
                <br />
                <ul>
                  {userPartOfLists.map((list) => (
                    ...
                    >
                      <li
                        className={`transition-all duration-200 cursor-pointer text-2xl font-black p-3 mb-3 rounded-lg flex-grow text-center text-[#F1FAEE] max-w-2xl overflow-auto ${
                          activeListItem === list.id ? 'bg-[#457B9D]' : 'bg-[#1D3557] hover:bg-[#457B9D]'
                        }`}
                        onClick={() => {
                          setActiveListItem(list.id)
                          setSelectedListID({
