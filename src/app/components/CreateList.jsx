...
export const CreateList = () => {
  ...
  const handleCreateList = () => {
    ...
    const listsRef = ref(database, `${userInfo.isAnonymous ? 'guests' : 'users'}/${userInfo.uid}/lists`)
      ...
      {showOverlay && userInfo && (
        <div className='fixed top-0 left-0 w-full h-full bg-[rgba(60,84,150,0.48)] flex justify-center items-center'>
          <div className={'z-1 bg-white rounded-xl p-10 w-full max-w-lg'}>
            <div className='popup '>
              <p className='text-center text-7xl font-black mb-6 '>What is the list&apos;s name?</p>
              <input
                type='text'
              ...
              <div className='flex'>
                <label className={` mx-2 bg-slate-200 rounded-2xl ${userInfo.isAnonymous ? 'w-full' : 'w-1/2'}`}>
                  <input
                    className='w-full'
                    type='radio'
                    name='listType'
              ...
              {listType === 'group' && (
                <div className='mt-4'>
                  <h3 className='text-lg font-semibold mb-2 text-center'>List Members (separate with comma):</h3>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const membersArray = inputValue ? inputValue.split(',').map((member) => member.trim()) : []
